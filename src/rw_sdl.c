/* $Id: rw_sdl.c,v 1.16 2003/01/09 12:06:13 jaq Exp $
 *
...
static int joy_numbuttons;
static int jx, jy, jt, js;
static int lr_axis, ud_axis, throttle_axis, strafe_axis;
#endif
...
void RW_IN_Init(in_state_t *in_state_p)
{
	...
	j_invert_y = ri.Cvar_Get("j_invert_y", "0", 0);
	lr_axis = (int) ri.Cvar_Get("j_lr_axis", "2", CVAR_ARCHIVE)->value;
	ud_axis = (int) ri.Cvar_Get("j_ud_axis", "3", CVAR_ARCHIVE)->value;
	throttle_axis = (int) ri.Cvar_Get("j_throttle", "1", CVAR_ARCHIVE)->value;
	strafe_axis = (int) ri.Cvar_Get("j_strafe", "0", CVAR_ARCHIVE)->value;
#endif
	my_freelook = ri.Cvar_Get( "freelook", "1", 0);
...
void RW_IN_Commands (void)
{
	...
	for (i = 0; i < joy_numbuttons; i++) {
		...
		key_index = (i < 12) ? K_XBOX_A : K_AUX17;
...
void RW_IN_Move (usercmd_t *cmd)
{
  ...
  if (joystick_avail && joy) {
    ...
    else
	{
	  ...
	  cmd->sidemove += m_side->value * (js/100);
	}
...
void GetEvent(SDL_Event *event)
{
	...
	case SDL_JOYBUTTONDOWN:
	  keyq[keyq_head].key = 
	    ((((SDL_JoyButtonEvent*)event)->button < 12)?K_XBOX_A:K_AUX17)+
	...
	case SDL_JOYHATMOTION:
		...
		if (hat&SDL_HAT_UP) 
			in_state->Key_Event_fp(K_DPAD_UP, 1);
		else if (SDL_HAT_UP & old_hat) 
			in_state->Key_Event_fp(K_DPAD_UP, 0);

		if (hat&SDL_HAT_DOWN   ) 
			in_state->Key_Event_fp(K_DPAD_DOWN, 1);
		else if (SDL_HAT_DOWN & old_hat) 
			in_state->Key_Event_fp(K_DPAD_DOWN, 0);

		if (hat&SDL_HAT_LEFT   ) 
			in_state->Key_Event_fp(K_DPAD_LEFT, 1);
		else if (SDL_HAT_LEFT & old_hat)
			in_state->Key_Event_fp(K_DPAD_LEFT, 0);

		if (hat&SDL_HAT_RIGHT ) 
			in_state->Key_Event_fp(K_DPAD_RIGHT, 1);
		else if (SDL_HAT_RIGHT & old_hat)
			in_state->Key_Event_fp(K_DPAD_RIGHT, 0);
...
extern cvar_t * vid_xbox_xpos;       /* window position x */
extern cvar_t * vid_xbox_ypos;       /* window position y */
extern cvar_t * vid_xbox_xstretch;       /* window position x */
extern cvar_t * vid_xbox_ystretch;       /* window position y */

void SWimp_EndFrame (void)
{
	/* SDL_Flip(surface); */

	SDL_XBOX_SetScreenPosition(vid_xbox_xpos->value, vid_xbox_ypos->value);
	SDL_XBOX_SetScreenStretch(vid_xbox_xstretch->value, vid_xbox_ystretch->value);
...
// get events from x server
	...
	if (joystick_avail && joy) {
	  ...
	  js = SDL_JoystickGetAxis(joy, strafe_axis);
	}
#endif	
	mouse_buttonstate = 0;
